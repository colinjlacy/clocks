...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function register($username, $password, $email, $additional_data = array(), $groups = array(), $ip)
	{
...
//		$ip_address = $this->_prepare_ip($this->input->ip_address());
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if (empty($identity) || empty($password)) // this would only happen if there was a problem between the two functions, which would be weird.  But I'm leaving it in anyway.  For no particular reason.
		{
			$data['error'] = "You need to enter both a Username and a Password.";
		...
		$query = $this->db->select($this->identity_column . ', username, email, id, first_name, last_name, active, last_login')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		                  ->get($this->tables['users']);


		if($this->is_time_locked_out($identity)) // check to make sure the user isn't locked out
		{
			$this->trigger_events('post_login_unsuccessful');
			$this->set_error('login_timeout');

			$data['error'] = "Your login attempt has timed out.";
		...
		if ($query->num_rows() === 1) // check to make sure a row was actually returned from the database
		{
			...
			if ($password === TRUE) // the (hashed) sent password matches the password in the database
			{
				if ($user->active == 0) // the user is not active in the database... for whatever reason that may be...
				{
					$this->trigger_events('post_login_unsuccessful');
			...
			{
				$data['error'] = "Your password did not match the password for this username";
